    ...
    <div class="pt-3">
        ...
        @foreach ($functs as $funct)
            ...
            <div class="">
                ...
                @foreach ($funct->sub_functs()->where('filter', 'LIKE', $filter)->where('funct_id', $funct->id)->where('user_type', 'office')->where('type', 'opcr')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $sub_funct)
                    <div>
                        <h5>
                            {{ $sub_funct->sub_funct }}
                        </h5>

                        @foreach ($sub_funct->outputs()->where('filter', 'LIKE', $filter)->where('type', 'opcr')->where('user_type', 'office')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $output)
                ...
                @endforeach
                <div>
                    @foreach ($funct->outputs()->where('filter', 'LIKE', $filter)->where('type', 'opcr')->where('user_type', 'office')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $output)
