    ...
    <section class="section pt-3">
        @foreach ($functs as $funct)
            ...
            <div class="">
                ...
                @if ($duration)
                    @foreach ($funct->sub_functs()->where('funct_id', $funct->id)->where('user_type', 'office')->where('type', 'opcr')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $sub_funct)
                        <div>
                            <h5>
                                @if (($duration && $duration->end_date >= date('Y-m-d')))
                            ...
                            @foreach ($sub_funct->outputs()->where('type', 'opcr')->where('user_type', 'office')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $output)
                    ...
                    @endforeach
                    <div>
                        @foreach ($funct->outputs()->where('type', 'opcr')->where('user_type', 'office')->where('duration_id', $duration->id)->where('added_by', '!=', null)->get() as $output)
